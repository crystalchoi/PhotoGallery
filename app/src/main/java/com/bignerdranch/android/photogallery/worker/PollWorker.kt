package com.bignerdranch.android.photogallery.worker
...
class PollWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        if (query.isEmpty()) {
            Log.i(TAG, "Query is empty, do NOTHING, return success")
        ...
        return try {
            ...
            if (items.isNotEmpty()) {
                ...
                if (newResultId == lastId) {
                    Log.i(TAG, "Query is NOT empty, Still have the same result: $newResultId")
                } else {
                    Log.i(TAG, "Query is NOT empty, Got a new result: $newResultId")
            ...
            Log.e(TAG, "Query is NOT empty, Background update failed", ex)
            Result.failure()
        }
    }
